...
Shows that ΔGE ≈ ΔFE + drop for each observation.

Since GE_pred = FE_pred − drop, it follows algebraically that:
    ΔGE = measured − GE_pred = measured − (FE_pred − drop) = ΔFE + drop

For each star, two bars side-by-side:
  Left:  stacked bar — |ΔFE| (bottom, lime) + drop (top, orange)
  Right: solid bar   — |ΔGE| (pink)

If the relationship holds, the stacked bar and the GE bar are the same height.
"""
...
OUTPUT_DIR = Path(__file__).parent / "plots" / "cat_individuals"
DPI = 150
